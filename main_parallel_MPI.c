...
double update_phi_field(double rho_phi[]) {
	...
	if (rank != size-1) {
		my_phi_size =  Lx/size;
		my_Amatrix_size = Lx*Lx/size;
	} else {
		my_phi_size = Lx - (Lx/(size))*(size - 1);
		my_Amatrix_size = Lx*Lx - (Lx*Lx/(size))*(size - 1);
	}
	double *my_sub_phi = malloc(my_phi_size*sizeof(double));
	...
	for (counts_index = 0; counts_index < size; counts_index++) {
		counts_phi[counts_index] = Lx/size;
		counts_Amatrix[counts_index] = Lx*Lx/size;
		if (counts_index == size - 1) {
			counts_phi[counts_index] = Lx - (Lx/(size))*(size - 1);
	...
	while (resid_norm > tol && counter < 1000) {
        ...
        for (k=0; k<counts_phi[rank]; k++) {
            sum_mp = 0; sum_norm = 0;
            for (l=0; l<Lx; l++) {
                sum_mp += (double)my_sub_Amatrix[k*Lx + l]*phi_new[l];
            }
            my_matrix_prod_sub[k] = sum_mp;
        }
